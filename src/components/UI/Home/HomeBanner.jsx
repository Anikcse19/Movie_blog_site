...
const HomeBanner = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`${base_url}/articles/suggestions`).then((res) => {
      if (res?.data) {
      ...
      >
        {!articles.length > 0 && (
          <div className="w-full lg:w-full h-full animate-pulse bg-gray-700 p-3 flex gap-2">
            <div className="w-[200px] h-[100px] bg-gray-400 animate-pulse"></div>
          ...
          reverseSuggestionMovies.slice(0, 3).map((suggestionMovie) => (
            ...
            >
              ...
              <div className="w-[65%] flex flex-col justify-between gap-1 pt-3">
                ...
                <span
                  className={` font-[700] ${
                    isHover.value === suggestionMovie.id
                      ? "text-red-600 text-[15px] xl:text-[15px]"
