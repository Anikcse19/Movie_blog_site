...
const SingleTagContentPage = () => {
    ...
    useEffect(()=>{
      ...
      axios.get(`${base_url}/articles?tag_ids[]=${id}`).then(res=>setContent(res?.data?.articles))
    ...
    <Layout>
     ...
     <div className="w-[90%] lg:w-full mx-auto my-5">
      ...
      <div className=" text-white inline-block mt-2 ">
       <span className="text-red-600 text-base lg:text-2xl">Searched Tag:</span> <span className="text-black font-bold text-base lg:text-2xl py-2 px-3">{currentTag?.name}</span>
      </div>
      </div>
...
export default SingleTagContentPage
